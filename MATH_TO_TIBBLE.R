...
one_density<-function(A,v=1,poi=1000,pp=200, n=1024){
  A<-A %>% select(all_of(v))
  tmp<-unnest(A,cols = 1)
  ran<-range(tmp$x)
...
mul_density<-function(A,v=c(1,2),poi=1000,pp=200, n=256){
  A<-A %>% select(all_of(v))
  NVARS<-length(v)
  DF1<-one_density(A,v=1,poi=poi,pp=pp, n=n)
...
plots_strips_distr<-function(B,v=c(1,2),n=64,poi=500,pp=200){

  plots<-list()
  require(RColorBrewer)
 
  col_strip <- brewer.pal(9, "Reds")
  df2<-mul_density(B,v = v,n=n,poi=poi,pp = pp)
